...
function PRiSM_plot_bourdet(canvas, data, opts) {
    ...
    function render() {
        ...
        // ── Slope guides ─────────────────────────────────────────────
        // Anchored to the ACTUAL DATA so the user can see at a glance
        // whether each regime is present:
        //   • WBS (slope 1) line passes through the EARLIEST derivative
        //     point — if the data follows it for a decade, that's WBS.
        //   • Radial (slope 0) line passes through the LATE-TIME derivative
        //     stabilisation level — should be flat where the data is flat.
        //   • Linear (½) and bilinear (¼) anchored at the geometric mean
        //     of the derivative range (intermediate regimes — user moves
        //     mentally to see if any data segment lies along them).
        ctx.save();
        PRiSM_plot_clip(ctx, plot.x, plot.y, plot.w, plot.h);

        // Compute anchor points from the actual data derivative.
        var earlyAnchor = null, lateAnchor = null;
        if (drPts.length >= 2) {
            // First valid derivative point = WBS anchor.
            earlyAnchor = { x: drPts[0][0], y: drPts[0][1] };
            // Late-time stabilisation = arithmetic mean of last 25% of
            // derivative points (in log space, since the derivative is
            // log-distributed at late time).
            var nLate = Math.max(2, Math.floor(drPts.length * 0.25));
            var sumLog = 0;
            for (var li = drPts.length - nLate; li < drPts.length; li++) {
                sumLog += Math.log(drPts[li][1]);
            }
            lateAnchor = {
        ...
        var midX = Math.sqrt(scaleX.min * scaleX.max);
        var midY = Math.sqrt(scaleY.min * scaleY.max);
        ...
        slopes.forEach(function (s) {
            // y = anchor.y * (x / anchor.x)^m  in log-log space.
            var xL = scaleX.min, xR = scaleX.max;
            var yL = s.anchor.y * Math.pow(xL / s.anchor.x, s.m);
            var yR = s.anchor.y * Math.pow(xR / s.anchor.x, s.m);
            ctx.strokeStyle = s.color;
            ctx.lineWidth = (s.m === 1.0 || s.m === 0.0) ? 1.5 : 1.0;
            ctx.setLineDash([5, 4]);
            ctx.beginPath();
            ctx.moveTo(tr.toX(xL), tr.toY(yL));
            ctx.lineTo(tr.toX(xR), tr.toY(yR));
            ctx.stroke();
            ctx.setLineDash([]);
            // Anchor marker (small circle) so the user sees where the line is pinned.
            ...
            ctx.textAlign = 'right';
            ctx.textBaseline = 'bottom';
            var ly = Math.max(plot.y + 2, Math.min(plot.y + plot.h - 2, tr.toY(yR)));
