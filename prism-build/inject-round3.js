...
if (startIdx !== -1) {
  ...
  if (endIdx === -1) {
    console.error('Found Round-3 START sentinel but no END — aborting');
    process.exit(1);
  }
  const blockEnd = endIdx + END.length;
  out = html.slice(0, startIdx) + START + '\n' + blob + '\n' + END + html.slice(blockEnd);
  console.log('[replace] Replaced existing Round-3 block');
} else {
  const anchorIdx = html.indexOf(ANCHOR_PATTERN);
  if (anchorIdx === -1) {
    console.error('Could not find Round-2 END sentinel — aborting');
    process.exit(1);
  }
  ...
  console.log('[insert] Injected Round-3 block after Round-2 END (line ' +
